...
`tools/list` at `https://gateway.pipeworx.io/insee/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/insee_company \
  -H 'Content-Type: application/json' \
  -d '{"identifier":"552032534"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/insee_company`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
